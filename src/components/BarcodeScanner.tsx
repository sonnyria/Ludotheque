...
export const BarcodeScanner: React.FC<BarcodeScannerProps> = ({
  ...
  useEffect(() => {
    if (!googleTitle || googleTitle.trim().length < 2) {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`/api/games/google-suggest?q=${encodeURIComponent(googleTitle.trim())}`);
        if (res.ok) {
  ...
  const handleSelectGoogleSuggestion = (suggestedTitle: string) => {
    setGoogleSuggestions([]);
    setGoogleTitle('');
    stopCamera();
    if (onTitleSearch) {
      onTitleSearch(suggestedTitle);
    }
  };
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            {manualCode.trim().length >= 4 && (
              <a
                ...
                className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl border border-blue-500 transition flex items-center gap-1.5 shrink-0 shadow-sm active:scale-95"
          ...
          {manualCode.trim().replace(/\D/g, '').length >= 6 && (
            <div className="p-3 bg-white border border-blue-200 rounded-xl shadow-xs space-y-2">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Ex: Need for Speed Underground, Super Mario..."
                  value={googleTitle}
                  onChange={(e) => setGoogleTitle(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && googleTitle.trim().length >= 2) {
                      e.preventDefault();
                      handleSelectGoogleSuggestion(googleTitle.trim());
                    }
                  }}
                  className="w-full pl-3 pr-20 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                  type="button"
                  onClick={() => {
                    if (googleTitle.trim().length >= 2) {
                      handleSelectGoogleSuggestion(googleTitle.trim());
                    }
                  }}
                  disabled={googleTitle.trim().length < 2}
                  className="absolute right-1 top-1 bottom-1 px-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-md text-[11px] font-bold transition flex items-center gap-1 cursor-pointer"
                >
              ...
              {googleSuggestions.length > 0 && (
                <div className="bg-white border border-blue-200 rounded-lg shadow-md overflow-hidden divide-y divide-slate-100 max-h-40 overflow-y-auto">
                  <div className="px-2.5 py-1 bg-blue-50 text-[10px] font-bold text-blue-800 flex items-center justify-between">
                    <span>Suggestions Google :</span>
                    {isFetchingSuggestions && <Loader2 className="w-3 h-3 animate-spin text-blue-600" />}
                  </div>
                  {googleSuggestions.map((sug) => (
                    <button
                      key={sug}
                      type="button"
                      onClick={() => handleSelectGoogleSuggestion(sug)}
                      className="w-full text-left px-3 py-1.5 text-xs text-slate-800 hover:bg-blue-50 hover:text-blue-900 font-medium transition flex items-center justify-between group cursor-pointer"
                    >
                      <span className="truncate">{sug}</span>
                      <span className="text-[10px] text-blue-600 opacity-0 group-hover:opacity-100 font-bold shrink-0">
                        Choisir →
                      </span>
                    </button>
                  ))}
